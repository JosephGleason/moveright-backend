...
squat_camera_api = Namespace('squat_camera', description='Camera operations')
...
@squat_camera_api.route('/capture')
...
@socketio.on('connect', namespace='/squat_camera')
def handle_connect():
    print('[WEBSOCKET] Client connected to camera namespace')

# Handle WebSocket disconnection
@socketio.on('disconnect', namespace='/squat_camera')
def handle_disconnect():
    print('[WEBSOCKET] Client disconnected from squat camera namespace')
...
@socketio.on('request_frame', namespace='/squat_camera')
def handle_frame_request(data):
    ...
    try:
        ...
        if user_id not in active_cameras:
            socketio.emit('error', {'message': 'No active camera session'}, namespace='/squat_camera')
            return

        camera = active_cameras[user_id]

        if not camera.is_running():
            socketio.emit('error', {'message': 'Camera not running'}, namespace='/squat_camera')
            return

        #interchanging regular encoded frames with squat frames
        frame = camera.get_squat_frame()

        if frame is None:
            socketio.emit('error', {'message': 'No frame available'}, namespace='/squat_camera')
            return
        ...
        try:
            ...
            socketio.emit('error', {'message': str(e)}, namespace='/squat_camera')
            return
        socketio.emit('frame', {'image': frame_base64}, namespace='/squat_camera')
...
@socketio.on('start_stream', namespace='/squat_camera')
def handle_start_stream(data):
    ...
    try:
        ...
        socketio.emit('stream_started', {'status': 'streaming'}, namespace='/squat_camera')

    except Exception as e:
        socketio.emit('error', {'message': str(e)}, namespace='/squat_camera')

# Stop continuous video streaming
@socketio.on('stop_stream', namespace='/squat_camera')
def handle_stop_stream(data):
    ...
    try:
        ...
        socketio.emit('stream_stopped', {'status': 'stopped'}, namespace='/squat_camera')
